...
def main():
    ...
    def preprocess_function(examples):
        ...
        if "label" in examples:
            if train_is_multilabel:
                result["labels"] = [
                    [float(l) for l in label.split(",")] for label in examples["label"]
                ]
            else:
                if label_to_id is not None:
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            if train_is_multilabel:
                if predictions is None:
                    predictions = 1 / (
                        1 + np.exp(-outputs.logits.detach().cpu().numpy())
                    )
                    references = batch["labels"].detach().cpu().numpy()
                else:
                    predictions = np.append(
                        predictions,
                        1 / (1 + np.exp(-outputs.logits.detach().cpu().numpy())),
                    )
        ...
        if train_is_multilabel:
            ...
            eval_metric = compute_metrics(predictions, references)
        else:
