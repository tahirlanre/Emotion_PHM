...
class MTLDataset(Dataset):
    ...
    def __init__(
        ...
        if args.sampling_type == "dynamic" and single_task_scores is None:
            raise Exception("Need single task scores to compute dynamic sampling")
    ...
    def sample_batches(self):
        if self.args.sampling_type == "size":
            ...
            self.instances = self.arrange_instances_dynamically(self.evaluation_metrics)
    ...
    def arrange_batches(self):
        if self.args.batch_type == "partition":
            ...
            self.arranged_instances = self.group_instances_forced_heterogeneous()
        elif self.args.sampling_type == "all":
            self.arranged_instances = flatten(self.all_instances)
        else:
            raise NotImplementedError(
                "Don't know how to process `batch_type`={}".format(self.args.batch_type)
            )
    ...
    def group_instances_homogeneous(self):
        ...
        final_instances = flatten(all_batches_homogeneous)  # unflatten the batches
    ...
    @staticmethod
    def random_generator(self, seq: list, group_size: int, shuffle: bool = True):
    ...
    def gather_last_batch(
        ...
        last_batch = [dataset_batches[-1] for dataset_batches in sampled_bunches]
        if self.args.sampling_type == "uniform":
            pass
    ...
    def sample_from_all_instances(self, number_to_sample_per_dataset: typing.List[int]):
    ...
    def arrange_instances_by_size(self):
        ...
        if self.instances_per_batch is None:
            ...
            size_proportions = size_proportions / np.array(sum(size_proportions))
            self.instances_per_batch = self.get_instances_numbers_from_proportions(
                size_proportions
            )
        assert sum(self.instances_per_batch) == self.batch_size, "rounding did not work"
        grouped_samples = self.sample_from_all_instances(self.instances_per_batch)
        return grouped_samples
    ...
    def arrange_instances_uniform(self):
        ...
        assert len(set(self.instances_per_batch)) == 1, "is not uniform, {}".format(
        ...
        for index, dataset in enumerate(dataset_list):
            ...
            while instances_to_add > 0:
                if instances_to_add > len(dataset):
                    ...
                    tensor_list.append(
                        tuple([tensor[:instances_to_add] for tensor in dataset.tensors])
                    )
                    instances_to_add -= instances_to_add
            new_list.append(
                TensorDataset(
                    *[torch.cat(tensors, dim=0) for tensors in zip(*tensor_list)]
                )
            )
        return new_list
...
class MTLRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if (
            ...
            self.data_source.instances = self.data_source.arrange_instances_by_size()
